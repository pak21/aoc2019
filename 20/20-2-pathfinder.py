...
outer_portals = set(portal_locations[True].values())
...
states = [(0, (start_location, 0))]
min_distances = {(start_location, 0): 0}
min_solution = None

while states:
    distance, pos_and_level = heapq.heappop(states)

    if min_solution and distance >= min_solution:
        # Already found a solution smaller than this, we can stop now
        break

    if pos_and_level in min_distances and distance > min_distances[pos_and_level]:
        # Already been to this spot with a shorter distance so ignore this
        continue

    pos, level = pos_and_level

    for next_pos, steps in portal_distances[pos].items():
        next_distance = distance + steps

        if next_pos == end_location:
            if level == 0 and (not min_solution or next_distance < min_solution):
                min_solution = next_distance
        elif next_pos == start_location:
            # Do nothing
            pass
        else:
            level_change = -1 if next_pos in outer_portals else 1
            if level > 0 or level_change == 1:
                portal_end = portals[next_pos]
                next_distance += 1
                next_pos_and_level = (portal_end, level + level_change)
                if next_pos_and_level not in min_distances or next_distance < min_distances[next_pos_and_level]:
                    heapq.heappush(states, (next_distance, next_pos_and_level))
                    min_distances[next_pos_and_level] = next_distance
